...
function GetMatlabIndent(lnum)
  ...
  if getline(v:lnum) =~ '^\s*\(end\|else\|elseif\|case\|otherwise\|catch\)\>'
    ...
    if getline(plnum) =~ '^\s*\(for\|parfor\|if\|else\|elseif\|case\|while\|switch\|try\|otherwise\|catch\)\>'
...
"  endif
  ...
  elseif getline(plnum) =~ '^\s*\(for\|parfor\|if\|else\|elseif\|case\|while\|switch\|try\|otherwise\|catch\)\>'
